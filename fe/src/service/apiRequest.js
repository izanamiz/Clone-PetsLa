...
export const updateUser = async(accessToken, dispatch, newUser) => {
    ...
    try {
        const res = await axios.put(" http://127.0.0.1:8000/profile/", newUser, {
            headers: {
                Authorization: 'Bearer ' + accessToken,
            },
        })
...
export const getOrder = async(accessToken, dispatch) => {
    ...
    try {
        const res = await axios.get(" http://127.0.0.1:8000/get-order/", {
            headers: {
                Authorization: 'Bearer ' + accessToken,
            },
        })
...
export const getAddOrder = async(accessToken, dispatch, order) => {
    ...
    try {
        const res = await axios.post(" http://127.0.0.1:8000/add-order/", order, {
            headers: {
                Authorization: 'Bearer ' + accessToken,
            },
        });
